...
export type MusicLibrary = { tracks: MusicTrack[]; volume: number; repeat: boolean };
export type MusicImportResult = { library: MusicLibrary; failed: string[] };
export type MusicLibraryUpdate = { trackIds: string[]; volume: number; repeat: boolean };

export function emptyMusicLibrary(): MusicLibrary {
  return { tracks: [], volume: 0.7, repeat: false };
}
