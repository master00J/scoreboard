...
import { tSportBreakLabel, tSportLabel as tSportLabelBase, tSportPeriodName as tSportPeriodNameBase } from "./t-phase";
...
export function tSportLabel(t: TFunction, sport: unknown): string {
  ...
  return tSportLabelBase(t, profile.id, profile.label);
}
...
/** Knoplabel voor een periode in het bedieningspaneel: "Kwart 3", "Set 2", "Verlenging 1". */
export function tPeriodButton(t: TFunction, sport: unknown, period: number): string {
  ...
  return tSportPeriodNameBase(t, sport, period);
}

/** Periodenaam voor het stadionscherm (hoofdletters): "1E HELFT", "KWART 3", "SET 2", "VERLENGING". */
export function tPeriodName(t: TFunction, sport: unknown, period: number): string {
  const d = describePeriod(sport, period);
  const fallback = sportPeriodLabel(sport, period);
  switch (d.kind) {
    case "half":
      return tOr(t, d.index === 1 ? "sports.period.half1" : "sports.period.half2", fallback);
    case "overtime": {
      const profile = getSportProfile(sport);
      const extra = tOr(t, "sports.period.extra", "VERLENGING");
      return profile.maxOvertimePeriods > 1 ? `${extra} ${d.index}` : extra;
    }
    case "quarter":
      return tOr(t, "sports.period.quarter", fallback, { n: d.index });
    case "set":
      return tOr(t, "sports.period.set", fallback, { n: d.index });
    default:
      return tOr(t, "sports.period.quarter", fallback, { n: d.index });
  }
}
...
export function tBreakName(t: TFunction, sport: unknown): string {
  const translated = tSportBreakLabel(t, sport);
  return translated || sportBreakLabel(sport);
}
