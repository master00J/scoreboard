...
export type CommandResult = CommandAck;
...
async function updateState(db: Db, data: StateUpdate): Promise<DisplayState> {
  // De singleton-rij bestaat normaal (ensureBaseData), maar een commando mag nooit stranden op een lege DB.
  const existing = await db.displayState.findUnique({ where: { id: 1 }, select: { id: true } });
  if (!existing) await db.displayState.create({ data: { id: 1, mode: "IDLE" } });
...
async function resolveGoalIntroMediaId(db: Db): Promise<string | null> {
  const settingsRow = await db.appSettings.findUnique({ where: { id: 1 } });
  const introPath = settingsRow?.goalIntroVideoPath ?? null;
  if (introPath) {
    const fromPath = await ensureMediaItemForVideoPath(db, introPath, "Goal intro");
...
async function applyVolleyballDelta(
  ...
  if (!opts?.skipLog) {
    ...
    await logMatchEvent(db, m.id, {
      type: next.setJustWon ? "SET_WON" : next.technicalTimeout ? "TECHNICAL_TIMEOUT" : "POINT",
      minute: ctx.minute,
      period: m.currentPeriod,
      clockSec: 0,
      teamId: side === "home" ? m.homeTeamId : m.awayTeamId,
      meta: { side, delta, from: side === "home" ? m.homeScore : m.awayScore, sport: "VOLLEYBALL" },
  ...
  const stateData: StateUpdate = {
    mode: next.matchOver ? "FULLTIME" : s.mode === "FULLTIME" && !next.matchOver ? "MATCH" : s.mode,
  };
  ...
  switch (cmd.type) {
    ...
    case "match:setStatus": {
      ...
      if (match) {
        ...
        await updateState(db, {
          ...
          ...clearTimeoutClock(),
          postMatchStartedAt: isPostMatch ? (s.postMatchStartedAt ?? new Date()) : null,
          preMatchStartedAt: isPrematch ? (s.preMatchStartedAt ?? new Date()) : null,
        });
      }
      return { ok: true };
    }
    ...
    case "event:undo": {
      ...
      if (m) {
        ...
        } else if (ev.type === "POINT" || ev.type === "SET_WON") {
          if (profile.hasSets) {
