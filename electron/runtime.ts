 ...
 * monotone klok terwijl de wedstrijdklok loopt, dan verankeren we de klok opnieuw op de monotone tijd.
 */
    ...
    try {
      ...
      if (Math.abs(jumpMs) > 1000) {
        ...
        if (anyRunning) {
          ...
          // en start opnieuw vanaf nu, zodat er geen seconden bijkomen of verdwijnen.
    ...
    if (pathname === "/api/matches" && method === "POST") {
      ...
      const match = await prisma.match.create({
        data: {
          ...
          sport,
          currentPeriod: 1,
          servingSide: sportProfile.hasSets
            ? body.servingSide === "away"
              ? "away"
              : "home"
            : null,
          setFirstServer: sportProfile.hasSets
            ? body.servingSide === "away"
              ? "away"
              : "home"
            : null,
