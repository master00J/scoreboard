...
function Shell() {
  ...
  useEffect(()=>{
    if(!c.notice||c.notice.kind==='error'||c.notice.kind==='warning')return;
    const timer=setTimeout(c.dismissNotice,6000);return()=>clearTimeout(timer);
  },[c.notice]);
  ...
  return <SafeAreaView style={s.safe} edges={['top','left','right','bottom']}>
    ...
    {c.notice&&<View style={[s.notice,{borderLeftColor:c.notice.kind==='error'?colors.danger:c.notice.kind==='warning'?colors.warning:colors.accent}]} accessibilityLiveRegion="polite">
      <View style={{flex:1,gap:4}}><Text style={s.noticeText}>{c.notice.text||t(c.notice.key,c.notice.values)}</Text>
    ...
    <View style={s.tabs}>{TABS.map(([id,icon])=><Pressable key={id} accessibilityRole="tab" accessibilityState={{selected:id===tab}} accessibilityLabel={t(`nav.${id}`)}
      onPress={()=>select(id)} style={[s.tab,id===tab&&s.activeTab]}><Text style={{fontSize:18,color:id===tab?colors.accent:colors.muted}}>{icon}</Text><Text style={[s.tabText,id===tab&&{color:colors.accent}]}>{t(`nav.${id}`)}</Text></Pressable>)}</View>
  </SafeAreaView>;
}
